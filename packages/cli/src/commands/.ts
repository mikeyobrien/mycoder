...
export const command: CommandModule<SharedOptions, DefaultArgs> = {
  ...
  handler: async (argv) => {
    ...
    try {
      ...
      // Create a custom config with GitHub mode
      const customConfig = {
        ...
        getSystemPrompt: () => {
          ...
          } = require('mycoder-agent/dist/core/toolAgent/config.js');
          return getDefaultSystemPrompt({ githubMode: config.githubMode });
        },
      };

      // If GitHub mode is enabled, log it
      if (config.githubMode) {
        logger.info('GitHub mode is enabled');
      }

      const result = await toolAgent(prompt, tools, customConfig, {
        logger,
